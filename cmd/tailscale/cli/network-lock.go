...
var nlRemoveCmd = &ffcli.Command{
	...
	FlagSet: (func() *flag.FlagSet {
		...
		fs.BoolVar(&nlRemoveArgs.noResigning, "no-resigning", false, "do not resign signature which would be invalidated by removal of trusted signing keys")
		return fs
	})(),
}
	...
	if !nlRemoveArgs.noResigning {
		...
		for _, k := range removeKeys {
			...
			if bytes.Equal(st.PublicKey.KeyID(), kID) {
				return errors.New("cannot remove local trusted signing key while resigning; run command on a different node or with --no-resigning")
			}
		}
