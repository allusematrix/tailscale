...
func natConfigFromNetMap(nm *netmap.NetworkMap) *natV4Config {
	...
	for _, p := range nm.Peers {
		if !p.SelfNodeMasqueradeAddr.IsValid() {
			continue
		}
		...
		mak.Set(&dstMasqAddrs, peerV4, p.SelfNodeMasqueradeAddr)
		mak.Set(&listenAddrs, p.SelfNodeMasqueradeAddr, struct{}{})
	}
