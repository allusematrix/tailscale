...
func decodeWrappedAuthkey(key string, logf logger.Logf) (authKey string, isWrapped bool, sig *tka.NodeKeySignature, priv ed25519.PrivateKey) {
	...
	suffix := key[suffixIdx+len("--"):]
	sigIdx := strings.Index(suffix, "-")
	if sigIdx < 0 {
		logf("decoding wrapped auth-key: did not find delimiter")
		return key, false, nil, nil
	}
	if len(suffix[sigIdx:]) < 1+ed25519.PrivateKeySize {
		// Its really len(base64(priv-key)), but this guard checks
		// enough to avoid an index out of bounds.
		logf("decoding wrapped auth-key: private too short")
		return key, false, nil, nil
	}
	rawSig, err := base64.RawStdEncoding.DecodeString(suffix[:sigIdx])
	if err != nil {
		logf("decoding wrapped auth-key: signature decode: %v", err)
		return key, false, nil, nil
	}
	rawPriv, err := base64.RawStdEncoding.DecodeString(suffix[sigIdx+1:])
	if err != nil {
	...
	return key[:suffixIdx], true, sig, priv
}
